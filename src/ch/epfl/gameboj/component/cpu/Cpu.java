...
public final class Cpu implements Component, Clocked {
    ...
    private static final int PREFIXED_INSTRUCTIONS = 0xCB;
    ...
    private void reallyCycle() {
        if (IME && ieAndIfDiffZero()) {
            ...
            final Opcode opcode = opcodeValue == PREFIXED_INSTRUCTIONS ? PREFIXED_OPCODE_TABLE[read8AfterOpcode()] : DIRECT_OPCODE_TABLE[opcodeValue];
    ...
    private void dispatch(Opcode opcode) {
        ...
        case LD_A_N8R: {
            final int address = AddressMap.REGS_START + read8AfterOpcode();
            registerFile.set(Reg.A, read8(address));   
        } break;
        case LD_A_CR: {
            final int address = AddressMap.REGS_START + registerFile.get(Reg.C);
            registerFile.set(Reg.A, read8(address));   
        } break;
        case LD_A_N16R: {
        ...
        case LD_R8_N8: {
            final int data = read8AfterOpcode();
            final Reg reg = extractReg(opcode, 3);
            registerFile.set(reg, data);
        } break;
        case LD_R16SP_N16: {
            final int data = read16AfterOpcode();
            final Reg16 reg16 = extractReg16(opcode);
        ...
        case LD_R8_R8: {
            final Reg regH = extractReg(opcode, 3);
            final Reg regL = extractReg(opcode, 0);
        ...
        case ADD_A_R8: {
            ...
            final int vf = Alu.add(registerFile.get(Reg.A), registerFile.get(reg), c);
            setRegFlags(Reg.A, vf);
        } break;
        case ADD_A_N8: {
            final boolean c = combineBit3AndC(opcode);
            final int n8 = read8AfterOpcode();
            final int vf = Alu.add(registerFile.get(Reg.A), n8, c);
            setRegFlags(Reg.A, vf);
        } break;
        case ADD_A_HLR: {
            final boolean c = combineBit3AndC(opcode);
            final int hl = read8AtHl();
            final int vf = Alu.add(registerFile.get(Reg.A), hl, c);
            setRegFlags(Reg.A, vf);
        } break;
        case INC_R8: {
            final Reg reg = extractReg(opcode, 3);
            final int vf = Alu.add(registerFile.get(reg), 1);
            setRegFromAlu(reg, vf);
            combineAluFlags(vf, FlagSrc.ALU, FlagSrc.V0, FlagSrc.ALU, FlagSrc.CPU);
        } break;
        case INC_HLR: {
            final int vf = Alu.add(read8AtHl(), 1);
            final int v = Alu.unpackValue(vf);
            write8AtHl(v);
            combineAluFlags(vf, FlagSrc.ALU, FlagSrc.V0, FlagSrc.ALU, FlagSrc.CPU);
        } break;
        case INC_R16SP: {
            final Reg16 reg16 = extractReg16(opcode);
            final int vf = reg16.name() == "AF" ? Alu.add16H(SP, 1) : Alu.add16H(reg16(reg16), 1);
            final int v = Alu.unpackValue(vf);
            setReg16SP(reg16, v);
        } break;
        case ADD_HL_R16SP: {
            final Reg16 reg16 = extractReg16(opcode);
            final int vf = reg16.name() == "AF" ? Alu.add16H(reg16(Reg16.HL), SP) : Alu.add16H(reg16(Reg16.HL), reg16(reg16));
            final int v = Alu.unpackValue(vf);
            setReg16(Reg16.HL, v);
            combineAluFlags(vf, FlagSrc.CPU, FlagSrc.V0, FlagSrc.ALU, FlagSrc.ALU);
        } break;
        case LD_HLSP_S8: {
            final boolean addSP = !Bits.test(opcode.encoding, 4);
            int v = read8AfterOpcode();
            v = Bits.clip(16, Bits.signExtend8(v));
            final int vf = Alu.add16L(SP, v);
            final int result = Alu.unpackValue(vf);
            if (addSP) SP = result;
        ...
        case SUB_A_R8: {
            final boolean c = combineBit3AndC(opcode);
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.sub(registerFile.get(Reg.A), registerFile.get(reg), c);
            setRegFlags(Reg.A, vf);
        } break;
        case SUB_A_N8: {
            final boolean c = combineBit3AndC(opcode);
            final int n8 = read8AfterOpcode();
            final int vf = Alu.sub(registerFile.get(Reg.A), n8, c);
            setRegFlags(Reg.A, vf);
        } break;
        case SUB_A_HLR: {
            final boolean c = combineBit3AndC(opcode);
            final int hl = read8AtHl();
            final int vf = Alu.sub(registerFile.get(Reg.A), hl, c);
            setRegFlags(Reg.A, vf);
        } break;
        case DEC_R8: {
            final Reg reg = extractReg(opcode, 3);
            final int vf = Alu.sub(registerFile.get(reg), 1);
            setRegFromAlu(reg, vf);
            combineAluFlags(vf, FlagSrc.ALU, FlagSrc.V1, FlagSrc.ALU, FlagSrc.CPU);
        } break;
        case DEC_HLR: {
            final int vf = Alu.sub(read8AtHl(), 1);
            final int v = Alu.unpackValue(vf);
            write8AtHl(v);
            combineAluFlags(vf, FlagSrc.ALU, FlagSrc.V1, FlagSrc.ALU, FlagSrc.CPU);
        } break;
        case CP_A_R8: {
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.sub(registerFile.get(Reg.A), registerFile.get(reg));
            setFlags(vf);
        } break;
        case CP_A_N8: {
            final int n8 = read8AfterOpcode();
            final int vf = Alu.sub(registerFile.get(Reg.A), n8);
            setFlags(vf);
        } break;
        case CP_A_HLR: {
            final int hl = read8AtHl();
            final int vf = Alu.sub(registerFile.get(Reg.A), hl);
            setFlags(vf);
        } break;
        case DEC_R16SP: {          
            final Reg16 reg16 = extractReg16(opcode);
            final int v = reg16.name() == "AF" ? Bits.clip(16, SP - 1) : Bits.clip(16, reg16(reg16) - 1);
            setReg16SP(reg16, v);
        } break;
        ...
        case AND_A_N8: {
            final int n8 = read8AfterOpcode();
            final int vf = Alu.and(registerFile.get(Reg.A), n8);
            setRegFlags(Reg.A, vf);
        } break;
        case AND_A_R8: {
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.and(registerFile.get(Reg.A), registerFile.get(reg));
            setRegFlags(Reg.A, vf);
        } break;
        case AND_A_HLR: {
            final int hl = read8AtHl();
            final int vf = Alu.and(registerFile.get(Reg.A), hl);
            setRegFlags(Reg.A, vf);
        } break;
        case OR_A_R8: {
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.or(registerFile.get(Reg.A), registerFile.get(reg));
            setRegFlags(Reg.A, vf);
        } break;
        case OR_A_N8: {
            final int n8 = read8AfterOpcode();
            final int vf = Alu.or(registerFile.get(Reg.A), n8);
            setRegFlags(Reg.A, vf);
        } break;
        case OR_A_HLR: {
            final int hl = read8AtHl();
            final int vf = Alu.or(registerFile.get(Reg.A), hl);
            setRegFlags(Reg.A, vf);
        } break;
        case XOR_A_R8: {
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.xor(registerFile.get(Reg.A), registerFile.get(reg));
            setRegFlags(Reg.A, vf);
        } break;
        case XOR_A_N8: {
            final int n8 = read8AfterOpcode();
            final int vf = Alu.xor(registerFile.get(Reg.A), n8);
            setRegFlags(Reg.A, vf);
        } break;
        case XOR_A_HLR: {
            final int hl = read8AtHl();
            final int vf = Alu.xor(registerFile.get(Reg.A), hl);
            setRegFlags(Reg.A, vf);
        } break;
        case CPL: {
            final int v = Bits.complement8(registerFile.get(Reg.A));
        ...
        // Rotate, shift
        case ROTCA: {
            final RotDir dir = rotDirection(opcode);
            final int vf = Alu.rotate(dir, registerFile.get(Reg.A));
            setRegFromAlu(Reg.A, vf);
            combineAluFlags(vf, FlagSrc.V0, FlagSrc.V0, FlagSrc.V0, FlagSrc.ALU);
        } break;
        case ROTA: {
            final RotDir dir = rotDirection(opcode);
            final boolean c = registerFile.testBit(Reg.F, Flag.C);
            final int vf = Alu.rotate(dir, registerFile.get(Reg.A), c);
            setRegFromAlu(Reg.A, vf);
            combineAluFlags(vf, FlagSrc.V0, FlagSrc.V0, FlagSrc.V0, FlagSrc.ALU);
        } break;
        case ROTC_R8: {
            final RotDir dir = rotDirection(opcode);
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.rotate(dir, registerFile.get(reg));
            setRegFlags(reg, vf);
        } break;
        case ROT_R8: {
            final RotDir dir = rotDirection(opcode);
            final boolean c = registerFile.testBit(Reg.F, Flag.C);
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.rotate(dir, registerFile.get(reg), c);
            setRegFlags(reg, vf);
        } break;
        case ROTC_HLR: {
            final RotDir dir = rotDirection(opcode);
            final int vf = Alu.rotate(dir, read8AtHl());
            write8AtHlAndSetFlags(vf);
        } break;
        case ROT_HLR: {
            final RotDir dir = rotDirection(opcode);
            final boolean c = registerFile.testBit(Reg.F, Flag.C);
            final int vf = Alu.rotate(dir, read8AtHl(), c);
            write8AtHlAndSetFlags(vf);
        } break;
        case SWAP_R8: {
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.swap(registerFile.get(reg));
            setRegFlags(reg, vf);
        } break;
        case SWAP_HLR: {
            final int vf = Alu.swap(read8AtHl());
            write8AtHlAndSetFlags(vf);
        } break;
        case SLA_R8: {
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.shiftLeft(registerFile.get(reg));
            setRegFlags(reg, vf);
        } break;
        case SRA_R8: {
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.shiftRightA(registerFile.get(reg));
            setRegFlags(reg, vf);
        } break;
        case SRL_R8: {
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.shiftRightL(registerFile.get(reg));
            setRegFlags(reg, vf);
        } break;
        case SLA_HLR: {
            final int vf = Alu.shiftLeft(read8AtHl());
            write8AtHlAndSetFlags(vf);
        } break;
        case SRA_HLR: {
            final int vf = Alu.shiftRightA(read8AtHl());
            write8AtHlAndSetFlags(vf);
        } break;
        case SRL_HLR: {
            final int vf = Alu.shiftRightL(read8AtHl());
            write8AtHlAndSetFlags(vf);
        } break;

        // Bit test and set
        case BIT_U3_R8: {
            final int index = extractIndex(opcode);
            final Reg reg = extractReg(opcode, 0);
            final int vf = Alu.testBit(registerFile.get(reg), index);
            combineAluFlags(vf, FlagSrc.ALU, FlagSrc.V0, FlagSrc.V1, FlagSrc.CPU);
        } break;
        case BIT_U3_HLR: {
            final int index = extractIndex(opcode);
            final int vf = Alu.testBit(read8AtHl(), index);
            combineAluFlags(vf, FlagSrc.ALU, FlagSrc.V0, FlagSrc.V1, FlagSrc.CPU);
        } break;
        case CHG_U3_R8: {
            final Boolean set = bitResSet(opcode);
            final int index = extractIndex(opcode);
            final Reg reg = extractReg(opcode, 0);
        ...
        // Misc. ALU
        case DAA: {
            final int regAValue = registerFile.get(Reg.A);
            final int regFValue = registerFile.get(Reg.F);
            final boolean n = Bits.test(regFValue, Flag.N);
            final boolean h = Bits.test(regFValue, Flag.H);
            final boolean c = Bits.test(regFValue, Flag.C);

            final int vf = Alu.bcdAdjust(regAValue, n,  h, c);
            setRegFromAlu(Reg.A, vf);
    ...
    private int reg16(Reg16 r) {
        final int h = registerFile.get(Reg.values()[r.index() * 2]);
        final int l = registerFile.get(Reg.values()[r.index() * 2 + 1]);
    ...
    private Reg extractReg(Opcode opcode, int startBit) {
        final int a = Bits.extract(opcode.encoding, startBit, 3);
    ...
    private void combineAluFlags(int vf, FlagSrc z, FlagSrc n, FlagSrc h, FlagSrc c) {
        final FlagSrc[] flags = {z, n, h, c};
        final Flag[] flagList = Flag.values();
        //On détermine la valeur de chaque fanion selon la valeur de FlagSrc.
        for (int i = 0; i < flags.length; i++) {
            final int index = 7 - i; //On choisit l'index correspondant au bon fanions
            switch (flags[i].name()) {
            case ("V1") : registerFile.setBit(Reg.F, flagList[index], true); break;
            case ("V0") : registerFile.setBit(Reg.F, flagList[index], false); break;
            case ("ALU") : {
                final boolean flag = Bits.test(vf, index);
    ...
    private boolean checkCondition(Opcode opcode) {
        final int cc = Bits.extract(opcode.encoding, 3, 2);
        switch (cc) {     
